...
- **CI/CD**: Updated GitHub Actions workflow to use npm Trusted Publishers (OIDC)
  - Replaced NPM_TOKEN secret with id-token: write permission
  - More secure: short-lived OIDC tokens scoped to specific repo/workflow
  - No npm tokens need to be stored in GitHub secrets

### Infrastructure

- Added `.github/workflows/publish.yml` for automatic npm publishing on release
...
- `package.json` bin path corrected from `./dist/index.js` to `dist/index.js` (npm publish warning)
- `package.json` repository URL normalized to `git+https://` prefix
